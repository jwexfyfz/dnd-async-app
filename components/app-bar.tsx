...
export default function AppBar({ left, title }: { left?: React.ReactNode; title?: string }) {
  ...
  async function handleSignOut() {
    ...
    window.location.href = '/v2/setup';
  }

  return (
    <div className="grid grid-cols-3 items-center px-4 py-3 border-b border-slate-100 bg-white flex-shrink-0">
      <div>{left ?? <span />}</div>
      <div className="text-center">
        {title && <span className="text-sm font-semibold tracking-wide text-slate-800">{title}</span>}
      </div>
