...
export function SubclassPicker({ stats, onConfirm }: {
  ...
  useEffect(() => {
    const measure = () => setCardWidth(window.innerWidth - 80);
    measure();
  ...
  if (loading) {
    return (
      <div className="mb-4 rounded-xl border-2 border-amber-300 bg-amber-50 p-4 text-center text-sm text-amber-700">
        Loading subclasses…
      </div>
    );
  }
    ...
    return (
      <div className="mb-4 rounded-xl border-2 border-amber-300 bg-amber-50 overflow-hidden">
        <div className="px-4 py-2.5 bg-amber-100 border-b border-amber-200">
          <span className="text-sm font-bold text-amber-900">Level {pending?.type !== 'heroic_sacrifice' ? pending?.level : ''} — {activeSub.name}</span>
        </div>
        ...
        <div className="px-4 pb-3 flex gap-2">
          ...
          <button
            ...
            disabled={confirming}
            className="flex-1 py-2 bg-amber-500 text-white rounded-lg text-sm font-semibold disabled:opacity-40 hover:bg-amber-600 transition-colors"
          >
  ...
  const translateX = 40 - activeIdx * (cardWidth + 12) + dragOffset;
  ...
  return (
    <div className="mb-4 rounded-xl border-2 border-amber-300 bg-amber-50 overflow-hidden">
      ...
      <div className="pt-3 pb-1" style={{ overflow: 'hidden', width: '100%' }}>
        <div
          style={{
            display: 'flex',
            gap: 12,
          ...
          onTouchMove={e => {
            ...
            setDragOffset(Math.max(-(cardWidth + 12), Math.min(cardWidth + 12, dx)));
          }}
          onTouchEnd={() => {
            if (dragStartX !== null) {
              if (dragOffset < -40 && activeIdx < subclasses.length - 1) setActiveIdx(i => i + 1);
              else if (dragOffset > 40 && activeIdx > 0) setActiveIdx(i => i - 1);
            }
            setDragStartX(null);
            setDragStartY(null);
            setDragOffset(0);
        ...
        >
          {subclasses.map((sub, i) => (
            <div
              key={sub.key}
              style={{ width: cardWidth, maxWidth: cardWidth, flexShrink: 0, maxHeight: 300, overflowY: 'auto' }}
